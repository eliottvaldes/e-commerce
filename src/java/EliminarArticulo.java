            ...
            try{
                ...
                if(v.validarStock(id)==false){
                    ...
                    if (existe==false) {
                        ...
                        rs = set.executeQuery("select * from usuario where user_usr = '"+user_usr+"'");
                        boolean match= false;

                        while(rs.next()){
                            if (pas_usr.equals(rs.getString("pas_usr"))){
                                match = true;
                                break;
                            }
                        }
                        ...
                        if(match == false){
                            ...
                            String q = "delete from articulo where id_art = "+id_art;
                            if(set.executeUpdate(q)==1){
                           message = "<h2 class=\"tm-block-title mt-3\">Se elimino el articulo</h2>\n";
                            }else{
                                message = "<h2 class=\"tm-block-title mt-3\">Surgio un problema al realizar la actualizacion, intentalo de nuevo</h2>\n";
                            }
                        }
                    }
                }
                ...
                System.out.println(e.getMessage());
                System.out.println(e.getStackTrace());
                message = "<h2 class=\"tm-block-title mt-3\">Lo lamento, no puedes eliminar un articulo que tiene registros de compras</h2>\n";
            }
